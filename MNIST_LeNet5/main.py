...
def clip_tensor(input_tensor, eps, batch_size, min_data, max_data):
    ...
    # If the L2 norm of transformed image is greater than the eps, we scale it down by a factor of (eps / tensor_norm)
...
# Function to increases Sparsity Rate for all images of original test dataset
def sparsity_rate_increment(model, device, test_loader, num_classes, c_init, args):
    ...
    for index, (data, target) in enumerate(tqdm(test_loader, desc='Transformation Progress')):
        ...
        # To compute the minimum and maximum pixel values for each sample in the batch
        ...
        for i in range(i_max):
            ...
            if (i>0):
                pred_transformed = output[0].max(1, keepdim=False)[1]
                coeff = torch.stack([torch.where(init_pred[i] == pred_transformed[i], (coeff[i] + c_min)/2, (coeff[i] + c_max)/2) for i in range(args.batch_size)])
            
            # To compute L-CrossEntropy independently for each image in the batch
            l_ce = torch.stack([F.cross_entropy(output[0][j], init_pred[j]) for j in range(args.batch_size)])
            ...
            # To updates x manually (not via optimizer.step()), so we disable gradient tracking for that update
            with torch.no_grad():
            ...
            # To re-enable gradient tracking for X in the next loop
            x.requires_grad_(True)

        # To store the transformed data (x_transformed) in a similar dataset
        x_transformed = x.detach()
        if args.store_transformation:
            for i in range(x_transformed.size(0)):
                transformed_data.append((x_transformed[i], target[i]))
        
        ### Transformation Ends Here ###

        # To inject the transformed Image to model again to compute the final accuracy and sparsity rate
    ...
    transformed_dataset = Transformed_Dataset(transformed_data)

    # Calculate overall accuracy of all test dataset after transformation
    first_acc = correct_before/float(num_processed_images)
    final_acc = correct_after/float(num_processed_images)

    return transformed_dataset, first_acc, final_acc, (total_net_zeros_before/total_net_sizes_before), (total_net_zeros_after/total_net_sizes_after)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--constrained', action='store_true', help="To enable clipping the purturbed data")
    parser.add_argument('--store_transformation', action='store_true', help="To store the transformed images")
    parser.add_argument('--power', action='store_true', help="To generate power results for CNVLUTIN architecture")
    parser.add_argument('--transformed', action='store_true', help="To test the transformed data rather than clean data")
...
# Transformation:   python3 main.py --phase sparsity-transform --eps 1 --eps_iter 0.2 --imax 500 --beta 22 --batch_size 10 --img_end_index 200 --constrained --store_transformation --dataset mnist_dataset --weights weights/mnist_13_0.9896.pkl
